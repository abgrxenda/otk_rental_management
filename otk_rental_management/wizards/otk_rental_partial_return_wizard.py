...
class OtkRentalPartialReturnWizard(models.TransientModel):
    _name = 'otk.rental.partial.return.wizard'
    ...
    line_ids = fields.One2many(
        'otk.rental.partial.return.wizard.line',
        'wizard_id',
        'Items to Return'
    )
    ...
    def default_get(self, fields_list):
        ...
        if project_id:
            ...
            rented_serials = self.env['otk.rental.equipment.serial'].search([
    ...
    def action_confirm_return(self):
        ...
        remaining_rented = self.env['otk.rental.equipment.serial'].search_count([
            ('current_project_id', '=', self.project_id.id),
            ('status', '=', 'rented')
        ])
        remaining_reserved = self.env['otk.rental.equipment.serial'].search_count([
            ('current_project_id', '=', self.project_id.id),
            ('status', '=', 'reserved')
        ])
...
class OtkRentalPartialReturnWizardLine(models.TransientModel):
    _name = 'otk.rental.partial.return.wizard.line'
    _description = 'Partial Return Line'

    wizard_id = fields.Many2one(
        'otk.rental.partial.return.wizard',
        required=True,
        ondelete='cascade'
    )
    ...
    def _onchange_condition(self):
        ...
        if self.condition == 'good':
            ...
            self.damage_fee = float(param.get_param('otk_rental.damage_minor_threshold', 100))
        elif self.condition == 'damaged':
            # Suggest moderate damage fee from settings
            param = self.env['ir.config_parameter'].sudo()
            self.damage_fee = float(param.get_param('otk_rental.damage_moderate_threshold', 500))
