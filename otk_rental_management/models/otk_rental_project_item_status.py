...
class OtkRentalProjectItemStatus(models.Model):
    _name = 'otk.rental.project.item.status'
    ...
    def _compute_display_name(self):
        ...
        for record in self:
            if record.serial_id:
                record.display_name = f"[{record.equipment_name}] {record.serial_number} - {dict(record._fields['status'].selection).get(record.status)}"
            else:
                record.display_name = f"[{record.equipment_name}] Qty: {record.quantity} - {dict(record._fields['status'].selection).get(record.status)}"
