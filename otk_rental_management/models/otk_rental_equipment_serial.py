...
class OtkRentalEquipmentSerial(models.Model):
    _name = 'otk.rental.equipment.serial'
    ...
    def action_view_status_history(self):
        ...
        return {
            ...
            'res_model': 'otk.rental.project.item.status',
            'view_mode': 'list,form',
...
# ========== QR CODE METHODS ==========
    ...
    def create(self, vals_list):
        """Auto-generate serial number if not provided, then generate QR codes"""
        next_sequence_by_equipment = {}
        for vals in vals_list:
            if not vals.get('serial_number'):
                equipment = self.env['otk.rental.equipment'].browse(vals.get('equipment_id'))
                if equipment.auto_generate_serials:
                    # Generate serial: EQUIPMENT_CODE-XXXX
                    code = equipment.code or 'EQ'
                    if equipment.id not in next_sequence_by_equipment:
                        next_sequence_by_equipment[equipment.id] = len(equipment.serial_ids) + 1
                    sequence = next_sequence_by_equipment[equipment.id]
                    next_sequence_by_equipment[equipment.id] += 1
                    vals['serial_number'] = f"{code}-{sequence:04d}"

        records = super(OtkRentalEquipmentSerial, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        # know each serial's current_project_id for the log entry)
        if 'status' in vals:
            for serial in self:
                if serial.current_project_id:
                    self.env['otk.rental.project.item.status'].create({
        ...
        result = super(OtkRentalEquipmentSerial, self).write(vals)
    ...
    def action_download_qr_code(self):
        ...
        return {
            ...
            'url': f'/web/content/otk.rental.equipment.serial/{self.id}/qr_code/{self.qr_code_filename}?download=true',
            'target': 'self',
        }
    ...
    def action_print_qr_code(self):
        ...
        return self.env.ref('otk_rental_management.action_report_qr_code_label').report_action(self)
