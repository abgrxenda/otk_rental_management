...
class OtkRentalEquipment(models.Model):
    _name = 'otk.rental.equipment'
    ...
    category_ids = fields.Many2many(
        ...
        'otk_rental_equipment_category_rel',
        'equipment_id',
        'category_id',
        string='Categories'
    )
    ...
    def action_view_otk_rental_history(self):
        """Open rental history for this equipment"""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Rental History'),
            'res_model': 'otk.rental.project.item',
            'view_mode': 'list,form',
            'domain': [('equipment_id', '=', self.id)],
        }
